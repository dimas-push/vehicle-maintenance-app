...
-- At most one loan per vehicle, enforced by the schema itself (UNIQUE below)
-- rather than only the app layer as before — a rebuild-time upgrade, not a
-- ported bug. Remaining balance is estimated as monthly_payment × months
-- remaining — a simplification that ignores interest amortization, not a
-- substitute for the lender's real payoff statement.
